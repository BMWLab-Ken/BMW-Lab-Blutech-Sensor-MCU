...
void OnChannelCheckTimerEvent( void )
{
    ...
    if( LoRaMacSetNextChannel( ) == 0 )
    {
      debug_str("    2:if( LoRaMacSetNextChannel( ) == 0 )\r\n");
        //if( ( LoRaMacState & MAC_TX_RUNNING ) == MAC_TX_RUNNING )//•d¶Ì??
        //{
    ...
    switch( macHdr->Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            if( fOpts != NULL )
            {
                ...
                {
                    LoRaMacBuffer[pktHeaderLen++] = fOpts[i];//∑sºWfOpts
                }
            }
            if( ( MacCommandsBufferIndex + fCtrl->Bits.FOptsLen ) <= 15 )//≠Y¡Ÿ¶≥™≈∂°,piggyback command
            {
                ...
                if( MacCommandsInNextTx == true )//Change in AddMacommand , yc
                {
                    ...
                    for( i = 0; i < MacCommandsBufferIndex; i++ )
                    {
                        ...
                        LoRaMacBuffer[pktHeaderLen++] = MacCommandsBuffer[i];// ¶bBuffer∑sºWMac Command
...
//debug_buf( fPort, 1);
                ...
                //LoRaMacMemCpy( LoRaMacPayload, LoRaMacBuffer + pktHeaderLen + 1, fBufferSize );  20160525 Ken •[±Kmarked ±N≠Ï•ª≥Q•[±K´·™∫LoRaMacPayload™Ω±µßÔ¶®•º•[±K™∫fBuffer(AppData)
...
uint8_t LoRaMacSendFrameOnChannel( ChannelParams_t channel )
{
    ...
    TimerStart( &MacStateCheckTimer );
        ...
        // Send now
	// Radio.Write( 0x01, 0x03);	//ÂèØ‰ª•‰∏çÂä†ÈÄôË°å
	Radio.Send( LoRaMacBuffer, LoRaMacBufferPktLen );
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {debug_str("onRxDone FRAME_TYPE_DATA_UNCONFIRMED_DOWN\r\n");
                    ...
                    ( micRx == mic ) )
                {
                    ...
                    if( fCtrl.Bits.FOptsLen > 0 )//∏—ΩXMacCommand
                    {
                    ...
                    if( ( ( size - 4 ) - appPayloadStartIndex ) > 0 )
                    {
                        ...
                        if( port == 0 )
                        {
                            ...
                            LoRaMacProcessMacCommands( LoRaMacRxPayload, 0, frameLen );//∏—ΩXMacCommand??
                        }
                        else
                        {
                            LoRaMacPayloadDecrypt( payload + appPayloadStartIndex,
...
*/
		// jim+-, 20150914		ÁÇ∫‰∫ÜÊñπ‰æøÔºåÂÖàÊîπÊàêÈÄôÊ®£
		Radio.SetChannel( 868000000 );
