...
void SX1276SetRx( uint32_t timeout )
{
    ...
    case MODEM_LORA:
        {
            ...
            else
            {
				...
				// SX1276Write( REG_LR_DIOMAPPING1, 0x00 );		//jim +-, 20150817
            }
    ...
    else
    {
        if( rxContinuous == true )
        {
            ...
            // Radio.Write( 0x01, 0x05);		//jim ++, 20150831
			SX1276SetOpMode( RFLR_OPMODE_RECEIVER );
        }
        else
        {
...
void SX1276SetTx( uint32_t timeout )
{
	...
	// SX1276Write( REG_OPMODE, 0x83 );					//jim ++
	SX1276Read(REG_LR_IRQFLAGS);						//jim ++, tx will fail if no this line, I don't know why	
}
...
void SX1276SetOpMode( uint8_t opMode )
{
    ...
    if( opMode != opModePrev )
    {
        ...
        {
            // SX1276SetAntSwLowPower( true );			//jim --
			;
        }
        else
        {
            // SX1276SetAntSwLowPower( false );			//jim --
...
{
    // if( SX1276.Spi.Spi == NULL )			//jim --., 20150816
    // {
        // while( 1 );
    // }
...
{
    // SX1276WriteBuffer( addr, &data, 1 );		//jim+-
...
{
    // uint8_t data;
    // SX1276ReadBuffer( addr, &data, 1 );		//jim+-
    // return data;
...
uint8_t readReg (u1_t addr) {
    ...
    sFLASH_CS_HIGH();//hal_pin_nss(1);     //sFLASH_CS_LOW(); //
    return val;
}
