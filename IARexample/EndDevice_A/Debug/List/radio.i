...
static void opmode (u1_t mode) {
    writeReg(0x01, (readReg(0x01) & ~0x07) | mode);
}
...
static void configLoraModem () {
    sf_t sf = getSf(LMIC.rps);
        ...
        case BW125: mc1 |= 0x70; break;
        case BW250: mc1 |= 0x80; break;
        case BW500: mc1 |= 0x90; break;
        default:
            if(!(0)) hal_failed();
        }
        switch( getCr(LMIC.rps) ) {
        case CR_4_5: mc1 |= 0x02; break;
        case CR_4_6: mc1 |= 0x04; break;
        case CR_4_7: mc1 |= 0x06; break;
        case CR_4_8: mc1 |= 0x08; break;
        default:
            if(!(0)) hal_failed();
        }
...
static void configChannel () {
    ...
    u8_t frf = 0xD84CCC;	
    writeReg(0x06, (u1_t)(frf>>16));
    writeReg(0x07, (u1_t)(frf>> 8));
    writeReg(0x08, (u1_t)(frf>> 0));
}
...
static void configPower () {
    ...
    writeReg(0x09, (u1_t)(0x80|(pw&0xf)));
    writeReg(0x5A, readReg(0x5A)|0x4);
...
static void txfsk () {
    ...
    writeReg(0x01, 0x10); 
    if(!(readReg(0x01) == 0x10)) hal_failed();
    ...
    writeReg(0x40, 0x00|0x30|0x04);

    
    writeReg(0x32, LMIC.dataLen+1); 

    
    writeReg(0x00, LMIC.dataLen);
    writeBuf(0x00, LMIC.frame, LMIC.dataLen);
...
static void txlora () {
    ...
    opmodeLora();
    if(!((readReg(0x01) & 0x80) != 0)) hal_failed();
    ...
    writeReg(0x0A, (readReg(0x0A) & 0xF0) | 0x08); 
    configPower();
    
    writeReg(0x39, 0x34);
    
    
    writeReg(0x40, 0x40|0x30|0xC0);
...
static void starttx () {
    if(!((readReg(0x01) & 0x07) == 0x00)) hal_failed();
...
static void rxlora (u1_t rxmode) {
    ...
    writeReg(0x0C, (0x20|0x1)); 
    
    writeReg(0x23, 64);
    
    writeReg(0x33, readReg(0x33)|(1<<6));
    
    writeReg(0x1F, LMIC.rxsyms);
    
    writeReg(0x39, 0x34);
    
    
    writeReg(0x40, 0x00|0x00|0xC0);		
    
    writeReg(0x12, 0xFF);
    
    writeReg(0x11, ~rxlorairqmask[rxmode]);
...
static void rxfsk (u1_t rxmode) {
    ...
    if(!(rxmode == RXMODE_SINGLE)) hal_failed();
...
void radio_init () {
    ...
    hal_pin_rst(0); 
    hal_waitUntil(os_getTime()+((ostime_t)( ((s8_t)(1) * 32768) / 1000))); 
    hal_pin_rst(2); 
    hal_waitUntil(os_getTime()+((ostime_t)( ((s8_t)(5) * 32768) / 1000))); 

    opmode(0x00);

    
    u1_t v = readReg(0x42);
    if(!(v == 0x12)) hal_failed(); 
    
    rxlora(RXMODE_RSSI);
    while( (readReg(0x01) & 0x07) != 0x05 ); 
    for(int i=1; i<16; i++) {
        for(int j=0; j<8; j++) {
            u1_t b; 
            while( (b = readReg(0x2C) & 0x01) == (readReg(0x2C) & 0x01) );
    ...
    writeReg(0x3B, (readReg(0x3B) & 0xBF)|0x40);
    while((readReg(0x3B)&0x20) == 0x20){ ; }
...
static const u2_t LORA_RXDONE_FIXUP[] = {
    [FSK]  =     ((ostime_t)( ((s8_t)(0) * 32768) / 1000000)), 
    [SF7]  =     ((ostime_t)( ((s8_t)(0) * 32768) / 1000000)), 
    [SF8]  =  ((ostime_t)( ((s8_t)(1648) * 32768) / 1000000)), 
    [SF9]  =  ((ostime_t)( ((s8_t)(3265) * 32768) / 1000000)), 
    [SF10] =  ((ostime_t)( ((s8_t)(7049) * 32768) / 1000000)), 
    [SF11] = ((ostime_t)( ((s8_t)(13641) * 32768) / 1000000)), 
    [SF12] = ((ostime_t)( ((s8_t)(31189) * 32768) / 1000000)), 
};
...
void radio_irq_handler (u1_t dio) {
    ...
    if( (readReg(0x01) & 0x80) != 0) { 
        u1_t flags = readReg(0x12);
        if( flags & 0x08 ) {
            
            LMIC.txend = now - ((ostime_t)( ((s8_t)(43) * 32768) / 1000000)); 
		} else if( (flags & 0x40) || (flags & 0x80) ) { 
            ...
            LMIC.dataLen = (readReg(0x1D) & 0x04) ?
                readReg(0x22) : readReg(0x13);
            
            writeReg(0x0D, readReg(0x10)); 
            
            readBuf(0x00, LMIC.frame, LMIC.dataLen);
            
            LMIC.snr  = readReg(0x19); 
            LMIC.rssi = readReg(0x1A) - 125 + 64; 
        } else if( flags & 0x80 ) {
        ...
        u1_t flags1 = readReg(0x3E);
        u1_t flags2 = readReg(0x3F);
        if( flags2 & 0x08 ) {
